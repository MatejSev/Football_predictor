...
const convertDataToCsv = (data) =>
  Object.keys(data).map((matchId) => {
    const { stage, date, status, home, away, result, information, statistics, lineups } = data[matchId];
    ...
    const home_rating = lineups?.homeTeamAverageRating ?? '';
    const away_rating = lineups?.awayTeamAverageRating ?? '';

    return { matchId, stage, status, date, home, away, result, home_rating, away_rating, ...informationObject, ...statisticsObject };
  });
